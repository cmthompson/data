...
import os
import RamanTools3 as RamanTools
...
def takeout(a,centers = (452,479),demo = False):
    ...
    for center in centers:
        ...
        x = a.index[center-7:center+7]
        y = a.values[center-7:center+7]
        
        guess= [a.values[center]-a.values[center+7],float(a.index[center]),20,float(a.values[center+7])]
...
def Fig3():  ### Combine final spectra from fig1 and fig2.  Display side by side.  Showing no SH stretch but some modes for phenyl ring in MTP.  
    ...
    annotate('C-S-H bend',(910,1.25),xytext = (910,2.0),arrowprops = {'width':1,'headwidth':3,'frac':0.05,'color':'k'},fontsize = 14 )
    annotate('Ring expansion',(804,2.1),xytext = (804,2.6),arrowprops = {'width':1,'headwidth':3,'frac':0.05,'color':'k'},fontsize = 14 )
    annotate('Ring expansion',(1095,1.85),xytext = (1140,1.9),arrowprops = {'width':1,'headwidth':3,'frac':0.05,'color':'k'},fontsize = 14 )
    annotate('Ring asymmetric rocking',(1600,1.7),xytext = (1300,1.8),arrowprops = {'width':1,'headwidth':3,'frac':0.05,'color':'k'},fontsize = 14 )
    ...
    annotate('S-H stretch',(2556,1.4),xytext = (2556,1.6),fontsize = 14,arrowprops = {'width':1,'headwidth':3,'frac':0.05,'color':'k'} )
...
def Fig6():
    ...
    legend()
    ### this data is in transmission mode
    ...
    largeAu2[:]-=glass[:]
    autobaseline(largeAu1,rnge,order = 0)
    largeAu1.plot(color = 'b',label = '535 sample1')
    ...
    #smallAu.plot(color = 'green',label = '525 nm sample')
    largeAu.plot(color= 'r',label = '535 nm sample, liquid')
    legend()
    ylabel('Absorbance (a.u.)')
    xlabel('Wavelength (nm)')
    
    
    ######Sample of films in integrating sphere.  
    
    #figure()
    r = loadtxt('/home/chris/Documents/DataWeiss/141203/integrating sphere for NP films.csv',
            unpack = True,
            delimiter = ',',
            skiprows = 2)
    largeAu1 = pandas.Series(r[5],r[4])
    largeAu2 = pandas.Series(r[7],r[6])
    largeAu =(largeAu2 +largeAu1)*60
    largeAu[:]-=largeAu.iloc[0]
    

    smallAu1 = pandas.Series(r[3],r[2])
    smallAu2 = pandas.Series(r[9],r[8])
    smallAu = smallAu1 + smallAu2
    

    
    #smallAu.plot(color = 'k',label = '525 nm sample')
    largeAu.plot(color= 'k',label = '535 nm sample, Film')
    legend()
    ylabel('Absorbance (a.u.)')
    xlabel('Wavelength (nm)')
